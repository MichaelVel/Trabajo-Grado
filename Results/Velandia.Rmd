...
estations <- cbind(Grupo = group, rlq1$mR) 
...
abundance_table <- abundance_metrics(data) %>%
    ...
    column_to_rownames() %>%
    select(EPT, Diptera, 
           Amphipoda, Bivalvia, Gastropoda, 
           Hemiptera, Hirudinea, Turbellaria, 
           Oligochaeta) %>%
    t() %>%
    as.data.frame() %>%
    round(digits = 2)
...
diversity_table <- data.frame(
                         'Shanon' = diversity(data, index = 'shannon'),
                         'Dominancia' = diversity(data, index = 'simpson'),
                          check.names = FALSE) %>%
                    round(digits = 2)
...
heatmap_abundance <- abundance_table %>%
                     rownames_to_column(var = 'Orden') %>%
                     pivot_longer(!matches("Orden"),
                                  names_to = "Estacion",
                                  values_to = "Abundancia")
                     

ggplot(heatmap_abundance, aes(Estacion, Orden, fill= Abundancia)) + 
  geom_tile()

dendogram + 
  tableGrob(diversity_table) + 
  tableGrob(abundance_table)+ 
