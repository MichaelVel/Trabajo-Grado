...
En el área de la bioindicación, los rasgos funcionales han ganado cada vez más
relevancia gracias a su relación clave con el funcionamiento de los ecosistemas.
En Colombia existen pocos estudios que aborden el tema, la mayoría de ellos en
tierras bajas. El objetivo del trabajo fue evaluar la respuesta de los rasgos 
funcionales de las comunidades de macroinvertebrados de ríos de la sabana de 
Bogotá a la variación en la calidad del agua y compararla con la respuesta de
indices bióticos conocidos. Se muestrearon macroinvertebrados de 11 estaciones
ubicadas en los ríos Frío y Neusa en el departamento de Cundinamarca, Colombia.
Se calcularon indices de diversidad (Shannon y Simpson) e indices bióticos 
(BMWP-Col, ASPT y FBI). Para la evaluación de la respuesta funcional al gradiente
ambiental se seleccionaron 30 categorías de  seis rasgos funcionales (a nivel de
familia) y se realizó un análisis de co-inercia RLQ, complementado con un análisis 
‘fourth corner’. Los resultados de los indices bioticos y de diversidad  no 
muestran una buena respuesta al gradiente ambiental observado. En comparación 
los resultados del análisis RLQ sugiere una relación mas clara  entre los rasgos 
funcionales de los macroinvertebrados con el gradiente ambiental. El análisis 
'fourth courner' sin embargo muestra que solo tres de las categorías presentan
correlaciones significativas con el gradiente ambiental. El presente trabajo 
soporta el potencial que tienen los rasgos funcionales como  complemento a los 
indices tradicionales de bioindicación en los sistemas lóticos de la Sabana de
Bogotá, sin embargo se sugieren nuevos estudios a nivel de  genero que 
contribuyan a profundizar en el estudio de la diversidad funcional de estos
sistemas.  

**Palabras Claves**: Diversidad Funcional, Índices, Riqueza, Grupos Funcionales  
\newpage

# ABSTRACT 

In Biomonitoring, functional traits have gained increasingly relevance because its
link with the ecosystem functioning. In Colombia there are few studies that 
address the topic, most of them in lowlands. The objective of the work was to 
evaluate the response of the functional traits of macroinvertebrate communities 
of streams in the 'Sabana de Bogotá' to the variation in water quality and 
compare it with the response of known biotic indices. Macroinvertebrates were 
sampled from 11 stations located in the 'Frio' and 'Neusa' rivers in the department
of Cundinamarca, Colombia. Diversity Indices (Shannon and Simpson) and Biotic 
Indices (BMWP-COL, ASPT and FBI) were calculated. For the evaluation of the 
functional response to the environmental gradient, 30 categories of six functional
traits (at family level) were selected and an combination of RLQ co-inertia analysis
and 'Fourth Corner' analysis were performed. The results of biotic and diversity
indications do not show a good response to the observed environmental gradient.
In comparison, the results of the RLQ analysis suggests a clearer ratio between 
the functional traits of the macroinvertebrates with the environmental gradient.
The 'Fourth Courner' analysis however shows that only three of the categories 
present significant correlations with the environmental gradient. The present 
work supports the potential of functional traits as a complement to traditional
biological index in the streams of the 'Sabana of Bogotá', however further studies 
at genus level of functional diversity in these systems are needed.

**Keywords**: Functional Diversity, Index, Richness, Functional Groups  
\newpage
...
de los ríos no variaron en gran medida entre las estaciones (Tabla 2 y Anexo 1)
...
variabilidad entre muestreos fue mucho más elevada (Anexo 1) que en el resto de 
...
abundance_table <- abundance_metrics(data) %>%
    rownames_to_column() %>%
    column_to_rownames() %>%
    select(Chironomidae, Simuliidae, Hyalellidae,
           Physidae, Sphaeriidae, Naididae) %>%
    round(digits = 2)
...
abundance_table %>%
  kbl(booktabs = TRUE, escape = FALSE, caption = "
        Abundancia relativa de las familias presentes en las estaciones 
...
eje del plano factorial (Anexo 3), lo cual confirma las tendencias observadas
en el PCA.
...
funcionales (Anexo 3), pero vale la pena mencionar algunas tendencias observadas 
...
```{r anexos_eda, message=FALSE, warning=FALSE, cache=FALSE,include = TRUE, echo = FALSE, fig.height= 8, fig.width= 9, fig.align='center', fig.cap= 'Anexo 1. El análisis univariado de las variables fisicoquimicas e hidrologicas. En las figuras se observan los diagramas de caja que corresponde a un análisis descriptivode las variables. Los gráficos cuantil-cuantil indican que tan bien se ajustan las variables a una distribución normal. '}
...
```{r manova,  message=FALSE, warning=FALSE, eval=FALSE}
results_manova <- list(
...
rasgos_func %>%
  kbl(booktabs = TRUE, escape = FALSE, caption = "
        Anexo 2. Clasificación de los rasgos funcionales  basados en 
...
res_4c <- as.data.frame(apply(res_4c, 2, 
      function(x) gsub("(?<=\\d\\.\\d{2}).*", '', x, perl = T)))


table_pdf <- expression(
 res_4c %>%
      kbl(booktabs = TRUE, escape = FALSE,
          caption = " Anexo 3. Resultados del Analisis Fourthcorner de las correlaciones
