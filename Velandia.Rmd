...
summary_data <- (function () {
    ...
    names <- c("Estacion", "Temperatura (C°)", "Ox disuelto (mg/L)", "pH",
             "Conductividad (µS/cm)", "Turbidez (NTU)", "Area (m²)",
    ...
    names(data) <-  names(summary_data$mean[1:9])
...
alimentan principalmente de algas y detritos finos (Fig 3 A y B) de pequeños
tamaños (<1cm) con cuerpos redondeados  y/o aplanados y que presentan
hábitos natatorios o excavadores (Fig 3 D, E y F). Cuando la acidez y la turbidez
del agua aumenta la tendencia favorece a los organismos trituradores y a los 
predadores (Fig 3 A y B) de forma cilíndrica y tamaño medio (1-2 cm) que se
encuentran sobre la superficie bentónica (Fig 3 D, E y F). La figura 3C muestra 
...
Trabajando en los indices BMWP-Col y el  ASPT con el proposito de contrastar los 
...
rasgos_func %>%
  kbl(booktabs = TRUE, escape = FALSE, caption = "
        Anexo1 : Clasificación de los rasgos funcionales  basados en 
...
```{r anexos_eda, message=FALSE, warning=FALSE, cache=FALSE,include = TRUE, echo = FALSE, fig.height= 6, fig.width= 6, fig.align='center'}
...
```{r anexos_eda2 , message=FALSE, warning=FALSE, cache=TRUE ,include = TRUE, echo = FALSE, fig.height= 9, fig.width= 6, fig.align='center'}
qq_plots <- univariate_graphical_eda(type_plot = 'quantile-normal')
box_plots <-  univariate_graphical_eda(type_plot = 'boxplot')
